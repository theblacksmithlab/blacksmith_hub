...
pub async fn news_block_creation<T: OpenAIClientInit + Send + Sync>(
    ...
    match &recipient {
        Recipient::Public => {
            ...
            processing_dialogs(&client, channels, app_state.clone(), user_tmp_dir.clone(), recipient).await?;
        }
        Recipient::Private(user_id) => {
            ...
            if let Some(pool) = db_pool {
                ...
                processing_chats(&client, chats, app_state.clone(), user_tmp_dir.clone(), recipient).await?;
            } else {
