...
pub async fn check_request_for_common_case<T: OpenAIClientInit + Send + Sync>(
    ...
    let system_role = match app_name {
        AppName::ProbiotBot => Some(AppsSystemRoles::Probiot(ProbiotRoleType::CommonCaseDetection)),
        ...
        AppName::BlacksmithWeb => Some(AppsSystemRoles::BlacksmithLab(BlacksmithLabRoleType::CommonCaseDetection)),
        _ => None,
    };
    ...
    let is_common: bool = match serde_json::from_str::<serde_json::Value>(&common_case_detection_result) {
        Ok(json) => json
            .get("is_common")
