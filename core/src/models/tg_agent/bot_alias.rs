...
impl GrootBotAlias {
    pub fn new(bot_id: i64, bot_username: String) -> Self {
        Self { bot_id, bot_username }
    }

    pub async fn check_bot_presence(&self, telegram_agent: &TelegramAgent, chat: &Chat) -> Result<bool> {
    ...
    pub async fn should_process_chat(&self, telegram_agent: &TelegramAgent, chat: &Chat) -> Result<bool> {
        ...
        info!("Chat {} is public and bot not present, processing", chat.id());
        Ok(true)
    }

    pub async fn get_chat_info_by_username(&self, telegram_agent: &TelegramAgent, chat_username: &str) -> Result<Option<(String, i64)>> {
        match telegram_agent.client.resolve_username(chat_username).await {
            ...
            Ok(None) => Ok(None),
            Err(e) => Err(anyhow::anyhow!("Could not get chat info for {}: {}", chat_username, e)),
        }
    }

    pub async fn send_message_to_bot(&self, telegram_agent: &TelegramAgent, message: &str) -> Result<()> {
        let bot_chat = telegram_agent.client.resolve_username(&self.bot_username).await?
            .ok_or_else(|| anyhow::anyhow!("Bot {} not found", self.bot_username))?;
        
        telegram_agent.client.send_message(bot_chat.pack(), message).await?;
