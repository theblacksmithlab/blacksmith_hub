...
pub async fn generate_podcast(
    ...
    info!(
        "Starting podcast generation for recipient: {:?}",
        recipient.clone()
    );
    ...
    info!(
        "Podcast for recipient: {:?} generated successfully: {:?}",
        recipient, podcast_path
    );
        ...
        match &recipient {
            ...
            Recipient::Private(uid) => {
                if let Some(db_pool) = app_state.core.db_pool.as_ref() {
                    ...
                    if user_channels.len() < 1 {
                        let msg = format!(
                            "📋 У тебя {} каналов для персонального подкаста.\n\n\
                            Для генерации подкаста нужны источники новостей, чтобы было из чего делать контент.\n\n\
                            Добавь каналы через меню \"⚙️ Настройки\" → \"📝 Управление каналами\"",
                            user_channels.len()
                        );
                ...
                let generated_podcast =
                    match generate_podcast(app_state.clone(), user_id, recipient.clone()).await {
                        Ok(path) => path,
                        Err(e) => {
                            error!(
                                "Failed to generate personal podcast for user {}: {:?}",
                                user_id, e
                            );
